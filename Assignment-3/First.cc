...
int main(int argc, char** argv) {
	// default tcpVersion (TCPNewReno)
	int tcpVersion = 0;

	// parse command-line arguments	
	CommandLine cmd;
	cmd.AddValue("tcpVersion", "TCP version to use (0-3)", tcpVersion);
	cmd.Parse(argc, argv);
	
	// set TCP version and name
	std::string name = "";
	switch(tcpVersion) {
		case 1:
			name = "TcpHighSpeed";
	...
	Config::SetDefault("ns3::TcpL4Protocol::SocketType", StringValue("ns3::" + name));
	...
	link.SetDeviceAttribute("DataRate", StringValue("8Mbps"));
    ...
    app->Setup(tcpSocket, recpAddress, 3000, DataRate("1Mbps"));
    ...
    app->SetStopTime(Seconds(30));
   	...
   	std::fstream file(name + ".csv", std::ios::out);
